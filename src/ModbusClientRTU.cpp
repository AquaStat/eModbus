...
ModbusClientRTU::ModbusClientRTU(int8_t rtsPin, uint16_t queueLimit)
    : ModbusClient(), MR_serial(nullptr), MR_lastMicros(micros()),
      MR_interval(2000), MR_rtsPin(rtsPin), MR_qLimit(queueLimit),
      MR_timeoutValue(DEFAULTTIMEOUT), MR_useASCII(false),
      MR_skipLeadingZeroByte(false) {
  if (MR_rtsPin >= 0) {
    pinMode(MR_rtsPin, OUTPUT);
    MTRSrts = [this](bool level) { digitalWrite(MR_rtsPin, level); };
    MTRSrts(LOW);
  } else {
...
ModbusClientRTU::ModbusClientRTU(RTScallback rts, uint16_t queueLimit)
    : ModbusClient(), MR_serial(nullptr), MR_lastMicros(micros()),
      MR_interval(2000), MTRSrts(rts), MR_qLimit(queueLimit),
      MR_timeoutValue(DEFAULTTIMEOUT), MR_useASCII(false),
      MR_skipLeadingZeroByte(false) {
  MR_rtsPin = -1;
  MTRSrts(LOW);
...
void ModbusClientRTU::doBegin(uint32_t baudRate, int coreID) {
  ...
  xTaskCreatePinnedToCore((TaskFunction_t)&handleConnection, taskName, 4096,
                          this, 6, &worker, coreID >= 0 ? coreID : NULL);
...
bool ModbusClientRTU::isModbusASCII() { return MR_useASCII; }
...
uint32_t ModbusClientRTU::pendingRequests() { return requests.size(); }
...
ModbusMessage ModbusClientRTU::syncRequestM(ModbusMessage msg, uint32_t token) {
  ...
  if (msg) {
    ...
    if (!addToQueue(token, msg, true)) {
      // No. Return error after deleting the allocated request.
      response.setError(msg.getServerID(), msg.getFunctionCode(),
                        REQUEST_QUEUE_FULL);
    } else {
...
// addBroadcastMessage: create a fire-and-forget message to all servers on the
// RTU bus
Error ModbusClientRTU::addBroadcastMessage(const uint8_t *data, uint8_t len) {
  ...
  // We do only accept requests with data, 0 byte, data and CRC must fit into
  // 256 bytes.
  if (len && len < 254) {
...
void ModbusClientRTU::handleConnection(ModbusClientRTU *instance) {
  ...
  while (1) {
    ...
    if (!instance->requests.empty()) {
      ...
      // Send it via Serial
      RTUutils::send(*(instance->MR_serial), instance->MR_lastMicros,
                     instance->MR_interval, instance->MTRSrts, request.msg,
      ...
      if (request.msg.getServerID() != 0 ||
          ((request.token & 0xFF000000) != 0xBC000000)) {
        // This is a regular request, Get the response - if any
        ModbusMessage response = RTUutils::receive(
            'C', *(instance->MR_serial), instance->MR_timeoutValue,
            instance->MR_lastMicros, instance->MR_interval,
            instance->MR_useASCII, instance->MR_skipLeadingZeroByte);
        ...
        if (response.size() > 1) {
          ...
          if (request.msg.getServerID() != response.getServerID()) {
            ...
            response.setError(request.msg.getServerID(),
                              request.msg.getFunctionCode(),
                              SERVER_ID_MISMATCH);
            // ServerID ok, but does the FC match as well?
          } else if (request.msg.getFunctionCode() !=
                     (response.getFunctionCode() & 0x7F)) {
            // No. Return error response
            response.setError(request.msg.getServerID(),
                              request.msg.getFunctionCode(), FC_MISMATCH);
          }
        } else {
